...
public class L7 extends JFrame implements KeyListener {
    ...
    public L7() {
        ...
        label2 = new JLabel ("Цыганков Михаил, ИКПИ-93");
        label = new JLabel ("Результат: 0"); // Эта метка хранит счет, инициализированный 0 баллами
        label1 = new JLabel ("Поехали!"); // Этот ярлык напоминает о состоянии игры: начало или конец
    ...
    //Инициализации игрового интерфейса
    public void init() {
        ...
        for (int i = 0; i <grids.length; i ++) {// инициализируем панель
            for (int j = 0; j < grids[i].length; j++) {
                ...
                if (j == 0 || j == grids[i].length - 1 || i == grids.length - 1) {
                    grids[i][j].setBackground(Color.getHSBColor(200, 200, 200));
                    data[i][j] = 1;
                }
        ...
        // Инициализация панели описание игры
        ...
        // Добавление в форму основной панели и панели описания
        ...
        this.setDefaultCloseOperation (JFrame.EXIT_ON_CLOSE); // Очистить форму
    }
    ...
    /* Сохраняем соответствующие данные неубывающего блока как 1, указывая, что в этой координате есть
   блок */
    ...
    public void removeRow(int row) {
        ...
        label.setText ("Результат: " + score); // Показать счет
    }
    /* Обновляем область главной панели игры после удаления блока строки */
    public void reflesh() {
        for (int i = 1; i < 25; i++) {
            for (int j = 1; j < 11; j++) {
                if (data [i] [j] == 1) {// Сделать квадрат синим там, где есть квадрат
    ...
    public void draw(int m, int n) {
        ...
        for (int i = 0; i <4; i ++) {// Проходим через 16 квадратов (4 * 4)
            for (int j = 0; j < 4; j++) {
                if ((temp & rect) != 0) {// Когда здесь квадрат
                    grids [m] [n] .setBackground (Color.PINK); // Место с квадратами становится синим
                }
                n ++; // Следующий столбец
                temp >>= 1;
            }
